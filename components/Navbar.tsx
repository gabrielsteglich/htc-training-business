          ...
          <div className="hidden md:flex space-x-8 items-center">
            ...
            <div className="relative group">
              ...
              <div className="absolute top-full right-0 pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                <div className="bg-white/95 backdrop-blur-sm shadow-xl rounded-lg overflow-hidden border border-slate-100 min-w-[100px]">
                  <button
                    onClick={toggleLanguage}
                    className="w-full px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 hover:text-aviation-navy transition-colors flex items-center gap-2"
                  >
                    <Globe className="w-3.5 h-3.5" />
                    <span className="font-semibold">{language === 'pt' ? 'US' : 'BR'}</span>
                    <span className="text-xs text-slate-400">{language === 'pt' ? 'English' : 'Português'}</span>
                  </button>
                </div>
              </div>
            </div>
